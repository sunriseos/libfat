...
impl Cluster {
    pub fn to_data_block_index<T>(&self, fs: &FatFileSystem<T>) -> BlockIndex where T: BlockDevice {
    ...
    pub fn to_fat_block_index<T>(&self, fs: &FatFileSystem<T>) -> BlockIndex where T: BlockDevice {
        let fat_offset = self.to_fat_offset();

        let fat_block_index = fs.partition_start.0 + fs.boot_record.reserved_block_count() as u32 + (fat_offset / Block::LEN_U32);
